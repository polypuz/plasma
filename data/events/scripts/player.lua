...
function Player:onLook(thing, position, distance)
        ...
        if self:getGroup():getAccess() then
                if thing:isItem() then
                        description = string.format("%s\nID Przedmiotu: %d", description, thing:getId())

                        local actionId = thing:getActionId()
                        if actionId ~= 0 then
                                description = string.format("%s, ID Akcji: %d", description, actionId)
                        end

                        local uniqueId = thing:getAttribute(ITEM_ATTRIBUTE_UNIQUEID)
                        if uniqueId > 0 and uniqueId < 65536 then
                                description = string.format("%s, Unikalne ID: %d", description, uniqueId)
                        end
                        ...
                        if transformEquipId ~= 0 then
                                description = string.format("%s\nZmienia sie na: %d (onEquip)", description, transformEquipId)
                        elseif transformDeEquipId ~= 0 then
                                description = string.format("%s\nZmienia sie na:: %d (onDeEquip)", description, transformDeEquipId)
                        end

                        local decayId = itemType:getDecayId()
                        if decayId ~= -1 then
                                description = string.format("%s\nRozpada sie: %d", description, decayId)
                        end
...
function Player:onLookInBattleList(creature, distance)
        local description = "Widzisz " .. creature:getDescription(distance)
...
function Player:onLookInTrade(partner, item, distance)
        self:sendTextMessage(MESSAGE_INFO_DESCR, "Widzisz " .. item:getDescription(distance))
end
