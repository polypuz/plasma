...
local config = {
	...
	--
	[4] = {female = 139, male = 131, nazwa = 'Knight', male_jeden = {{5880, 100},{5892, 1}}, male_dwa = {{5893, 100},{5887, 1},{5924, 1},{5885, 1}},
	female_jeden = {{5880, 100},{5892, 1}}, female_dwa = {{5893, 100},{5887, 1},{5924, 1},{5885, 1}}},
...
if (plec == PLAYERSEX_FEMALE) then
	wyglad= config[choiceId].female
	
elseif (plec == PLAYERSEX_MALE) then
	wyglad= config[choiceId].male		
end
		
local function dodatek_jeden()
	local condition = Condition(CONDITION_OUTFIT)
	condition:setTicks(10  * 1000)
	condition:setOutfit({lookType = wyglad, lookAddons = 1, lookHead = playerOutfit.lookHead, lookBody = playerOutfit.lookBody, lookLegs = playerOutfit.lookLegs, lookFeet = playerOutfit.lookFeet})
	player:addCondition(condition)
	player:say("Dodatek nr 1", TALKTYPE_MONSTER_SAY)
end

local function dodatek_dwa()
	local condition = Condition(CONDITION_OUTFIT)
	condition:setTicks(10  * 1000)
	condition:setOutfit({lookType = wyglad, lookAddons = 2, lookHead = playerOutfit.lookHead, lookBody = playerOutfit.lookBody, lookLegs = playerOutfit.lookLegs, lookFeet = playerOutfit.lookFeet})
	player:addCondition(condition)
	player:say("Dodatek nr 2", TALKTYPE_MONSTER_SAY)
end

local function dodatki()
	local condition = Condition(CONDITION_OUTFIT)
	condition:setTicks(10  * 1000)
	condition:setOutfit({lookType = wyglad, lookAddons = 3, lookHead = playerOutfit.lookHead, lookBody = playerOutfit.lookBody, lookLegs = playerOutfit.lookLegs, lookFeet = playerOutfit.lookFeet})
	player:addCondition(condition)
	player:say("Pelny outfit", TALKTYPE_MONSTER_SAY)
end
		if player:removeMoney(300) then
			addEvent(dodatek_jeden, 1*1000)		
			addEvent(dodatek_dwa, 11*1000)
			addEvent(dodatki, 21*1000)
		else
			player:showTextDialog(5919, "Musisz miec przy sobie 300 gp, aby przetestowac dodatek.")
		end
