...
void PrivateChatChannel::invitePlayer(const Player& player, Player& invitePlayer)
{
	...
	ss << player.getName() << " zaprasza Cie do " << (player.getSex() == PLAYERSEX_FEMALE ? "jej" : "jego") << " prywatnego czatu.";
	invitePlayer.sendTextMessage(MESSAGE_INFO_DESCR, ss.str());

	ss.str(std::string());
	ss << invitePlayer.getName() << (invitePlayer.getSex() == PLAYERSEX_FEMALE ? " zostala zaproszona" : " zostal zaproszony") << ".";
...
void PrivateChatChannel::excludePlayer(const Player& player, Player& excludePlayer)
{
	...
	std::ostringstream ss;
	ss << excludePlayer.getName() << ( excludePlayer.getSex() == PLAYERSEX_FEMALE ? ( " zostala wyrzucona.") : (" zostal wyrzucony.") );
	player.sendTextMessage(MESSAGE_INFO_DESCR, ss.str());
