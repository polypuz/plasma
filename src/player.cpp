...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	if (lookDistance == -1) {
		...
		if (group->access) {
			...
			/* s << " Jestes " << vocation->getVocDescription() << 'em.'; */
			if( vocation->getId() == 5){
				/* MS */
				/* MAGIEM, MAGIEM */
				/* s << " Jestes magiem."; */
				if( sex == PLAYERSEX_FEMALE ){
					s << " Jestes czarodziejka.";
				}
				else{
					s << " Jestes czarodziejem.";
				}
			}
			else if( vocation->getId() == 6){
				/* ED */
				/* DRUIDEM, DRUIDKA */
				if( sex == PLAYERSEX_FEMALE ){
					s << " Jestes druidka.";
		...
		if (group->access) {
			...
			s << " jest " << vocation->getVocDescription() << '.';
		} else {
			s << " nie ma profesji.";
		}
		*/
		if( group->access){
			...
			/* original */
			/* s << ", to " << vocation->getVocDescription() << '.'; */
			/* s << " Jestes " << vocation->getVocDescription() << 'em.'; */
			if( vocation->getId() == 5){
				/* MS */
				/* MAGIEM, MAGIEM */
	...
	if (party) {
		...
		if( invitationCount == 0 ){
			...
			s << " i " << invitationCount << " zaproszonymi.";
		}
	}
	...
	if (guild) {
		...
		if (rank) {
			...
			s << rank->name << " gildii " << guild->getName();
			if (!guildNick.empty()) {
				s << " (" << guildNick << ')';
			}
			...
			if (memberCount == 1) {
				...
				s << ", ktora ma " << memberCount << " czlonkow, z czego " << guild->getMembersOnline().size() << " zalogowanych.";
			}
			*/
			if( lookDistance == -1 ){
				s << " Nalezysz do gildii ";
			}
			else{
				s << " Nalezy do gildii ";
			}
...
void Player::addSkillAdvance(skills_t skill, uint64_t count)
{
	...
	while ((skills[skill].tries + count) >= nextReqTries) {
		...
		ss << "Awansowales " << getSkillName(skill) << " na poziom " << skills[skill].level << '.';
	...
	if (value != -1) {
		...
		if (!isLogin) {
			...
			if (lastQuestlogUpdate != currentFrameTime && g_game.quests.isQuestStorage(key, value, oldValue)) {
				...
				sendTextMessage(MESSAGE_EVENT_ADVANCE, "Twoja lista zadan zostala zaktualizowana.");
			}
		}
	} else {
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	const Tile* playerTile = player->getTile();
if (playerTile && (playerTile->hasFlag(TILESTATE_PROTECTIONZONE) || (player->getLevel() > 50))) {
		return false;
	}
...
bool Player::canWalkthroughEx(const Creature* creature) const
{
	...
	const Tile* playerTile = player->getTile();
return playerTile && (playerTile->hasFlag(TILESTATE_PROTECTIONZONE) || player->getLevel() < 50);
}
*/
...
{
	if (isNearDepotBox()) {
		sendTextMessage(MESSAGE_EVENT_ADVANCE, "Masz nowa poczte.");
	}
}
...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (isLogin && creature == this) {
		...
		std::cout << name << " sie zalogowal." << std::endl;
...
void Player::onAttackedCreatureDisappear(bool isLogout)
{
	...
	if (!isLogout) {
		sendTextMessage(MESSAGE_STATUS_SMALL, "Cel uciekl z pola widzenia.");
	}
}
...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		std::cout << getName() << " sie wylogowal." << std::endl;
	...
	if (!modalWindows.empty()) {
		...
		for (uint32_t modalWindowId : modalWindows) {
			if (modalWindowId == std::numeric_limits<uint32_t>::max()) {
				sendTextMessage(MESSAGE_EVENT_ADVANCE, "Trening offline zostal przerwany.");
				break;
			}
		}
		modalWindows.clear();
...
void Player::onThink(uint32_t interval)
{
	...
	if (!getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer()) {
		...
		if (idleTime > (kickAfterMinutes * 60000) + 60000) {
			...
			std::ostringstream ss;
			ss << "Byles bezczynny przez " << kickAfterMinutes << " minut. Jesli w ciagu najblizszej minuty sie nie wykonasz zadnej akcji, zostaniesz wyrzucony.";
...
void Player::removeMessageBuffer()
{
	...
	if (maxMessageBuffer != 0 && MessageBufferCount <= maxMessageBuffer + 1) {
		if (++MessageBufferCount > maxMessageBuffer) {
			...
			std::ostringstream ss;
			ss << "Zostales wyciszony na " << muteTime << " sekund.";
...
void Player::addManaSpent(uint64_t amount)
{
	...
	while ((manaSpent + amount) >= nextReqMana) {
		...
		std::ostringstream ss;
		ss << "Twoj poziom magiczny awansowal i wynosi teraz " << magLevel << '.';
		sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (sendText) {
		std::string expString = std::to_string(exp) + (exp != 1 ? " punktow doswiadczenia." : " punkt doswiadczenia.");

		TextMessage message(MESSAGE_EXPERIENCE, "Otrzymales " + expString);
		...
		if (!list.empty()) {
			...
			message.text = getName() + " otrzymal/a " + expString;
	...
	if (prevLevel != level) {
		...
		std::ostringstream ss;
		ss << "Awansowales na poziom " << level << '.';
		sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
	}
...
void Player::removeExperience(uint64_t exp, bool sendText/* = false*/)
{
	...
	if (sendText) {
		...
		std::string expString = std::to_string(lostExp) + (lostExp != 1 ? " punktow doswiadczenia." : " punkt doswiadczenia.");

		TextMessage message(MESSAGE_EXPERIENCE, "Straciles " + expString);
		message.position = _position;
		...
		if (!list.empty()) {
			message.type = MESSAGE_EXPERIENCE_OTHERS;
			message.text = getName() + " stracil " + expString;
			for (Creature* spectator : list) {
				spectator->getPlayer()->sendTextMessage(message);
			}
		}
	...
	if (oldLevel != level) {
		...
		std::ostringstream ss;
		ss << "Spadles z poziomu " << oldLevel << " na " << level << '.';
		sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
	}
...
void Player::death(Creature* _lastHitCreature)
{
	...
	if (skillLoss) {
		...
		if (expLoss != 0) {
			...
			if (oldLevel != level) {
				std::ostringstream ss;
				ss << "Spadles z poziomu " << oldLevel << " na poziom " << level << '.';
				sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
			}
...
Item* Player::getCorpse(Creature* _lastHitCreature, Creature* mostDamageCreature)
{
	...
	if (corpse && corpse->getContainer()) {
		std::ostringstream ss;
		if (_lastHitCreature) {
			ss << "Rozpoznajesz " << getNameDescription() << ". " << (getSex() == PLAYERSEX_FEMALE ? "Zostala ona zabita" : "Zostal on zabity") << " przez " << _lastHitCreature->getNameDescription() << '.';
		} else {
			ss << "Rozpoznajesz " << getNameDescription() << '.';
		}
...
void Player::notifyStatusChange(Player* loginPlayer, VipStatus_t status)
{
	...
	if (status == VIPSTATUS_ONLINE) {
		client->sendTextMessage(TextMessage(MESSAGE_STATUS_SMALL, loginPlayer->getName() + " sie zalogowal."));
	} else if (status == VIPSTATUS_OFFLINE) {
		client->sendTextMessage(TextMessage(MESSAGE_STATUS_SMALL, loginPlayer->getName() + " wylogowal sie."));
	}
}
...
{
	if (guid == vipGuid) {
		sendTextMessage(MESSAGE_STATUS_SMALL, "Nie mozesz dodac sam siebie.");
		return false;
	}

	if (VIPList.size() >= getMaxVIPEntries() || VIPList.size() == 200) { // max number of buddies is 200 in 9.53
		sendTextMessage(MESSAGE_STATUS_SMALL, "Twoja lista znajomych jest pelna.");
		return false;
	}

	auto result = VIPList.insert(vipGuid);
	if (!result.second) {
		sendTextMessage(MESSAGE_STATUS_SMALL, "Ten gracz jest juz na Twojej liscie.");
		return false;
	}
...
void Player::onAddCombatCondition(ConditionType_t type)
{
	switch (type) {
		case CONDITION_POISON:
			sendTextMessage(MESSAGE_STATUS_DEFAULT, "Jestes zatruty.");
			break;

		case CONDITION_DROWN:
			sendTextMessage(MESSAGE_STATUS_DEFAULT, "Dlawisz sie woda.");
			break;

		case CONDITION_PARALYZE:
			sendTextMessage(MESSAGE_STATUS_DEFAULT, "Jestes sparalizowany.");
			break;

		case CONDITION_DRUNK:
			sendTextMessage(MESSAGE_STATUS_DEFAULT, "Jestes pijany.");
			break;
...
void Player::onAttackedCreature(Creature* target)
{
	...
	/* this is so if player attacks summon w/o being in party or in guild with him */
	/* he will get a skull */
	else if( ! targetPlayer ){
		if( target->isSummon()){
			Creature* targetMaster = nullptr;
			...
			if( targetMaster->getPlayer() ){
				...
				if( masterPlayer && ! isPartner( masterPlayer ) && !isGuildMate( masterPlayer) && getPlayer() != masterPlayer ){
					...
					if( getSkull() == SKULL_NONE && getSkullClient( masterPlayer) == SKULL_YELLOW ){
						addAttacked( masterPlayer );
						masterPlayer->sendCreatureSkull( this );
					} else if( !masterPlayer->hasAttacked( this )){
						if( !pzLocked ){
							pzLocked = true;
							sendIcons();
						}
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	sendTextMessage(MESSAGE_EVENT_ADVANCE, "Uwaga! Zabojstwo " + attacked->getName() + " nie bylo prawnie usprawiedliwione.");
...
bool Player::addOfflineTrainingTries(skills_t skill, uint64_t tries)
{
	...
	if (skill == SKILL_MAGLEVEL) {
		...
		if (magLevel != currMagLevel) {
			std::ostringstream ss;
			ss << "Twoj magiczny poziom awansowal na " << magLevel << '.';
			sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
		}
	...
	std::ostringstream ss;
	ss << std::fixed << std::setprecision(2) << "Twoja umiejetnosc " << ucwords(getSkillName(skill)) << " zmienila sie z " << oldSkillValue << " (z " << oldPercentToNextLevel << "% postepu do nastepnego poziomu " << (oldSkillValue + 1) << ") na " << newSkillValue << " (z " << newPercentToNextLevel << "% postepu do nastepnego poziomu " << (newSkillValue + 1) << ')';
	sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
	return sendUpdate;
}
