...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!_player || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (!IOLoginData::preloadPlayer(player, name)) {
			disconnectClient("Twoja postac nie moze byc zaladowana.");
			return;
		}

		if (IOBan::isPlayerNamelocked(player->getGUID())) {
			disconnectClient("Twoja postac ma zablokowana nazwe.");
			return;
		}

		if (g_game.getGameState() == GAME_STATE_CLOSING && !player->hasFlag(PlayerFlag_CanAlwaysLogin)) {
			disconnectClient("Serwer jest w trakcie konserwacji.\nSprobuj za chwile, albo skontaktuj sie przez\nwww.mirkots.gimb.us");
			return;
		}
		...
		if (g_config.getBoolean(ConfigManager::ONE_PLAYER_ON_ACCOUNT) && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER && g_game.getPlayerByAccount(player->getAccount())) {
			disconnectClient("Mozesz byc zalogowany tylko na jedna postac.");
			return;
		}
		...
		if (!player->hasFlag(PlayerFlag_CannotBeBanned)) {
			...
			if (IOBan::isAccountBanned(accountId, banInfo)) {
				...
				if (banInfo.expiresAt > 0) {
					ss << "Twoje konto jest zablokowane do " << formatDateShort(banInfo.expiresAt) << " przez " << banInfo.bannedBy << ".\n\nPodany powod:\n" << banInfo.reason;
				} else {
					ss << "Twoje konto zostalo dozywotnio zablokowane przez " << banInfo.bannedBy << ".\n\nPowod:\n" << banInfo.reason;
				}
		...
		if (!WaitingList::getInstance()->clientLogin(player)) {
			...
			ss << "Zbyt wielu graczy jest online.\nJestes w kolejce na miejscu "
			   << currentSlot << " (oczekujacych).";
		...
		if (!IOLoginData::loadPlayerByName(player, name)) {
			disconnectClient("Twoja postac nie mogla byc zaladowana.");
			return;
		}
		...
		if (!g_game.placeCreature(player, player->getLoginPosition())) {
			if (!g_game.placeCreature(player, player->getTemplePosition(), false, true)) {
				disconnectClient("Pozycja swiatyni jest zle ustawiona. Skontaktuj sie z administratorem.");
				return;
			}
		}
		...
		if (eventConnect != 0 || !g_config.getBoolean(ConfigManager::REPLACE_KICK_ON_LOGIN)) {
			...
			disconnectClient("Nie mozesz zalogowac sie dwa razy.");
			return;
		}
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if (pos == std::string::npos) {
		disconnectClient("Musisz wpisac swoja nazwe uzytkownika.");
		return;
	}
	...
	if (version < CLIENT_VERSION_MIN || version > CLIENT_VERSION_MAX) {
		disconnectClient("Do zagrania wymagany jest klient wersji " CLIENT_VERSION_STR ", dostepny\ndo pobrania na\nwww.mirkots.gimb.us");
		return;
	}

	if (g_game.getGameState() == GAME_STATE_STARTUP) {
		disconnectClient("Mapa jest w trakcie zapisu. Sprobuj ponownie za 5 minut.");
		return;
	}

	if (g_game.getGameState() == GAME_STATE_MAINTAIN) {
		disconnectClient("Mapa jest w trakcie konserwacji. Sprobuj polaczyc sie za kilka minut.");
		return;
	}
	...
	if (IOBan::isIpBanned(getIP(), banInfo)) {
		if (banInfo.reason.empty()) {
			banInfo.reason = "(puste)";
		}

		std::ostringstream ss;
		ss << "Twoj adres IP jest zbanowany do " << formatDateShort(banInfo.expiresAt) << " przez " << banInfo.bannedBy << ".\n\nPowod:\n" << banInfo.reason;
		disconnectClient(ss.str());
		return;
	}
	...
	if (accountId == 0) {
		disconnectClient("Nazwa uzytkownika lub haslo\nnie zgadzaja sie.");
		return;
	}
...
void ProtocolGame::sendMarketDetail(uint16_t itemId)
{
	...
	if (it.abilities) {
		...
		if (it.abilities->stats[STAT_MAGICPOINTS] != 0) {
			...
			ss << "poziom magiczny " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
		}
		...
		if (it.abilities->speed != 0) {
			...
			ss << "predkosc " << std::showpos << (it.abilities->speed >> 1) << std::noshowpos;
		}
