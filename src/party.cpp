...
bool Party::leaveParty(Player* player)
{
	...
	player->sendTextMessage(MESSAGE_INFO_DESCR, "Opusciles druzyne.");
	...
	ss << player->getName() << " opuscil druzyne.";
...
bool Party::passPartyLeadership(Player* player)
{
	...
	std::ostringstream ss;
	ss << player->getName() << " jest teraz liderem druzyny.";
	...
	player->sendTextMessage(MESSAGE_INFO_DESCR, "Jestes teraz liderem druzyny.");
	return true;
}
...
bool Party::joinParty(Player& player)
{
	...
	std::ostringstream ss;
	ss << player.getName() << " dolaczyl do druzyny.";
	broadcastPartyMessage(MESSAGE_INFO_DESCR, ss.str());
	...
	ss.str(std::string());
	ss << "Dolaczyles do druzyny " << leaderName << ". Otworz druzynowy czat, aby porozumiewac sie z kompanami.";
...
void Party::revokeInvitation(Player& player)
{
	...
	ss << leader->getName() << (leader->getSex() == PLAYERSEX_FEMALE ? " odrzucila" : " odrzucil") << " swoje zaproszenie.";
	player.sendTextMessage(MESSAGE_INFO_DESCR, ss.str());

	ss.str(std::string());
	ss << "Zaproszenie " << player.getName() << " zostalo wycofane.";
...
bool Party::invitePlayer(Player& player)
{
	...
	ss << player.getName() << (player.getSex() == PLAYERSEX_FEMALE ? " zostala zaproszona." : "zostal zaproszony.");

	if (memberList.empty() && inviteList.empty()) {
		ss << " Otworz czat druzynowy, aby porozumiewac sie z kompanami.";
	...
	ss << leader->getName() << (leader->getSex() == PLAYERSEX_FEMALE ? " zaprosila" : " zaprosil") << " Cie do swojej druzyny.";
	player.sendTextMessage(MESSAGE_INFO_DESCR, ss.str());
	return true;
}
...
void Party::updateVocationsList()
{
	...
	if (size > 3) {
		...
		sharedExperienceMultiplier = 1.20f * 1.80f;
	} else if( size > 2){
		sharedExperienceMultiplier = 1.20f * 1.40f;
	} else if( size > 1){
		sharedExperienceMultiplier = 1.20f * 1.10f;
	} else {
		/* extraExpRate = 0.20f; */
		sharedExperienceMultiplier = 1.20f;
	}
...
bool Party::setSharedExperience(Player* player, bool _sharedExpActive)
{
	...
	if (sharedExpActive) {
		...
		if (sharedExpEnabled) {
			leader->sendTextMessage(MESSAGE_INFO_DESCR, "Wspoldzielenie doswiadczenia zostalo aktywowane.");
		} else {
			leader->sendTextMessage(MESSAGE_INFO_DESCR, "Wspoldzielenie doswiadczenia zostalo wlaczone, ale nie jest jeszcze aktywne.");
		}
	} else {
		leader->sendTextMessage(MESSAGE_INFO_DESCR, "Wspoldzielenie doswiadczenia zostalo deaktywowane.");
	}
