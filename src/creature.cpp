	...
	if (creature == this) {
		...
		if (!summons.empty()) {
			...
			for (Creature* summon : summons) {
				...
				if (Position::getDistanceZ(newPos, pos) > 2 || (std::max<int32_t>(Position::getDistanceX(newPos, pos), Position::getDistanceY(newPos, pos)) > 30) || newTile->getZone() != oldTile->getZone()) {
...
void Creature::onGainExperience(uint64_t gainExp, Creature* target)
{
	...
	TextMessage message(MESSAGE_EXPERIENCE_OTHERS, ucfirst(getNameDescription()) + " zyskal " + std::to_string(gainExp) + (gainExp != 1 ? " punktow doswiadczenia." : " punkt doswiadczenia."));
