...
Game::Game() :
	wildcardTree(false),
	offlineTrainingWindow(std::numeric_limits<uint32_t>::max(), "Wybierz umiejetnosc", "Musisz wybrac umiejetnosc:")
{
	...
	offlineTrainingWindow.choices.emplace_back("Walka mieczem oraz obrona tarcza", SKILL_SWORD);
	offlineTrainingWindow.choices.emplace_back("Walka toporem oraz obrona tarcza", SKILL_AXE);
	offlineTrainingWindow.choices.emplace_back("Walka obuchem oraz obrona tarcza", SKILL_CLUB);
	offlineTrainingWindow.choices.emplace_back("Walka na dystans oraz obrona tarcza", SKILL_DISTANCE);
	offlineTrainingWindow.choices.emplace_back("Magia i obrona tarcza", SKILL_MAGLEVEL);
	offlineTrainingWindow.buttons.emplace_back("OK", 1);
	offlineTrainingWindow.buttons.emplace_back("Anuluj", 0);
...
bool Game::playerBroadcastMessage(Player* player, const std::string& text) const
{
	...
	std::cout << "> " << player->getName() << " zakomunikowal: \"" << text << "\"." << std::endl;
...
void Game::playerOpenPrivateChannel(uint32_t playerId, std::string& receiver)
{
	...
	if (!IOLoginData::formatPlayerName(receiver)) {
		player->sendCancelMessage("Gracz o tej nazwie nie istnieje lub nie jest zalogowany.");
		return;
	}
                              ...
                              uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if (!tradePartner || tradePartner == player) {
		player->sendTextMessage(MESSAGE_INFO_DESCR, "Nie jest to mozliwe.");
		return;
	}

	if (!Position::areInRange<2, 2, 0>(tradePartner->getPosition(), player->getPosition())) {
		std::ostringstream ss;
		ss << tradePartner->getName() << " prosi, abys podszedl blizej.";
	...
	if (tradeItemContainer) {
		for (const auto& it : tradeItems) {
			Item* item = it.first;
			if (tradeItem == item) {
				player->sendTextMessage(MESSAGE_INFO_DESCR, "Ten przedmiot juz uczestniczy w wymianie.");
				return;
			}
	...
	if (tradeContainer && tradeContainer->getItemHoldingCount() + 1 > 100) {
		player->sendTextMessage(MESSAGE_INFO_DESCR, "Mozesz wymienic maksymalnie 100 przedmiotow.");
		return;
	}
...
bool Game::internalStartTrade(Player* player, Player* tradePartner, Item* tradeItem)
{
	...
	if (tradePartner->tradeState == TRADE_NONE) {
		std::ostringstream ss;
		ss << player->getName() << " chce z Toba handlowac.";
...
std::string Game::getTradeErrorDescription(ReturnValue ret, Item* item)
{
	if (item) {
		if (ret == RETURNVALUE_NOTENOUGHCAPACITY) {
			std::ostringstream ss;
			ss << "Nie masz wystarczajaco sily, aby uniesc";
			...
			std::ostringstream ss;
			ss << "Nie masz wystarczajaco duzo miejsca, aby schowac";
	...
	return "Wymiana nie mogla zostac dokonona.";
}
...
void Game::internalCloseTrade(Player* player)
{
	...
	player->sendTextMessage(MESSAGE_STATUS_SMALL, "Wymiana zostala anulowana.");
	player->sendTradeClose();
	...
	if (tradePartner) {
		...
		tradePartner->sendTextMessage(MESSAGE_STATUS_SMALL, "Wymiana zostala anulowana.");
		tradePartner->sendTradeClose();
	}
}
...
void Game::playerRequestAddVip(uint32_t playerId, const std::string& name)
{
	...
	if (!vipPlayer) {
		...
		if (!IOLoginData::getGuidByNameEx(guid, specialVip, formattedName)) {
			player->sendTextMessage(MESSAGE_STATUS_SMALL, "Gracz o tej nazwie nie jest zalogowany.");
			return;
		}

		if (specialVip && !player->hasFlag(PlayerFlag_SpecialVIP)) {
			player->sendTextMessage(MESSAGE_STATUS_SMALL, "Nie mozesz dodac tego gracza.");
			return;
		}
                     ...
                     const std::string& receiver, const std::string& text)
{
	...
	if (muteTime > 0) {
		std::ostringstream ss;
		ss << "Nie mozesz pisac jeszcze przez " << muteTime << " sekund.";
	...
	if (damage.primary.value > 0) {
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			if (!attacker) {
				...
				spectatorMessage += " zostal uleczony za " + damageString;
			} else {
				spectatorMessage += ucfirst(attacker->getNameDescription());
				spectatorMessage += " uleczyl ";
				if (attacker == target) {
					spectatorMessage += (targetPlayer ? (targetPlayer->getSex() == PLAYERSEX_FEMALE ? "sie" : "siebie") : "sie");
				} else {
					spectatorMessage += target->getNameDescription();
				}
				spectatorMessage += " za " + damageString;
			}
			...
			for (Creature* spectator : list) {
				...
				if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
					...
					message.text = "Leczysz " + target->getNameDescription() + " za " + damageString;
				} else if (tmpPlayer == targetPlayer) {
					message.type = MESSAGE_HEALED;
					if (!attacker) {
						message.text = (tmpPlayer->getSex() == PLAYERSEX_FEMALE ? ("Zostalas uleczona za " + damageString): ("Zostales uleczony za ") + damageString);
						/*message.text = "You were healed for " + damageString; */
					} else if (targetPlayer == attackerPlayer) {
						message.text = "Leczysz sie za " + damageString;
					} else {
						message.text = attacker->getNameDescription() + " leczy Cie za " + damageString;
					}
				} else {
		...
		if (target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				...
				std::string spectatorMessage = ucfirst(target->getNameDescription()) + " traci " + damageString + " many";
				if (attacker) {
					spectatorMessage += " przez ";
					if (attacker == target) {
						spectatorMessage += (targetPlayer ? (targetPlayer->getSex() == PLAYERSEX_FEMALE ? "jej wlasny atak" : "jego wlasny atak") : "jego wlasny atak");
					} else {
						spectatorMessage += "atak " + attacker->getNameDescription();
					}
				}
				spectatorMessage += '.';
				...
				for (Creature* spectator : list) {
					...
					if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
						...
						message.text = ucfirst(target->getNameDescription()) + " traci " + damageString + " many przez Twoj atak.";
					} else if (tmpPlayer == targetPlayer) {
						message.type = MESSAGE_DAMAGE_RECEIVED;
						if (!attacker) {
							message.text = "Tracisz " + damageString + " many.";
						} else if (targetPlayer == attackerPlayer) {
							message.text = "Tracisz " + damageString + " many przez swoj atak.";
						} else {
							message.text = "Tracisz " + damageString + " many przez atak " + attacker->getNameDescription() + '.';
						}
					} else {
		...
		if (message.primary.color != TEXTCOLOR_NONE || message.secondary.color != TEXTCOLOR_NONE) {
			...
			std::string spectatorMessage = ucfirst(target->getNameDescription()) + " traci " + damageString;
			if (attacker) {
				spectatorMessage += " przez ";
				if (attacker == target) {
					spectatorMessage += (targetPlayer ? (targetPlayer->getSex() == PLAYERSEX_FEMALE ? "jej wlasnego ataku" : "jego wlasnego ataku") : "jego wlasnego ataku");
				} else {
					spectatorMessage += "atak " + attacker->getNameDescription();
				}
			}
			...
			for (Creature* spectator : list) {
				...
				if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
					message.type = MESSAGE_DAMAGE_DEALT;
					message.text = ucfirst(target->getNameDescription()) + " traci " + damageString + " przez Twoj atak.";
				} else if (tmpPlayer == targetPlayer) {
					message.type = MESSAGE_DAMAGE_RECEIVED;
					if (!attacker) {
						message.text = "Tracisz " + damageString + '.';
					} else if (targetPlayer == attackerPlayer) {
						message.text = "Tracisz " + damageString + " przez swoj wlasny atak.";
					} else {
						message.text = "Tracisz " + damageString + " przez atak " + attacker->getNameDescription() + '.';
					}
				} else {
					message.type = MESSAGE_DAMAGE_OTHERS;
...
void Game::playerInviteToParty(uint32_t playerId, uint32_t invitedId)
{
	...
	if (invitedPlayer->getParty()) {
		std::ostringstream ss;
		ss << invitedPlayer->getName() << " juz jest w druzynie.";
		player->sendTextMessage(MESSAGE_INFO_DESCR, ss.str());
		return;
	}
...
void Game::playerJoinParty(uint32_t playerId, uint32_t leaderId)
{
	...
	if (player->getParty()) {
		player->sendTextMessage(MESSAGE_INFO_DESCR, "Jestes juz w druzynie.");
		return;
	}
...
void Game::sendGuildMotd(uint32_t playerId)
{
	...
	if (guild) {
		player->sendChannelMessage("Wiadomosc dnia", guild->getMotd(), TALKTYPE_CHANNEL_R1, CHANNEL_GUILD);
	}
}
...
void Game::playerReportBug(uint32_t playerId, const std::string& message, const Position& position, uint8_t category)
{
	...
	player->sendTextMessage(MESSAGE_EVENT_DEFAULT, "Twoje zgloszenie zostalo przeslane do " + g_config.getString(ConfigManager::SERVER_NAME) + ".");
}
...
void Game::playerAnswerModalWindow(uint32_t playerId, uint32_t modalWindowId, uint8_t button, uint8_t choice)
{
	...
	if (modalWindowId == std::numeric_limits<uint32_t>::max()) {
		if (button == 1) {
			...
			player->sendTextMessage(MESSAGE_EVENT_ADVANCE, "Trening offline anulowany.");
		}
