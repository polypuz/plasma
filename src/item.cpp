                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		if (it.runeLevel > 0 || it.runeMagLevel > 0) {
			...
			s << ". " << (it.stackable && tmpSubType > 1 ? "Moga" : "To moze") << " byc uzyte tylko przez ";
			...
			if (RuneSpell* rune = g_spells->getRuneSpell(it.id)) {
				...
				if (it.runeMagLevel > 0) {
					...
					s << it.runeMagLevel << " poziomem magicznym";
				}
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			s << " (Zasieg:" << static_cast<uint16_t>(item ? item->getShootRange() : it.shootRange);
			...
			if (attack != 0) {
				s << ", atak" << std::showpos << attack << std::noshowpos;
			}

			if (hitChance != 0) {
				s << ", szansa na traf. %" << std::showpos << static_cast<int16_t>(hitChance) << std::noshowpos;
			}
			...
			if (attack != 0) {
				...
				if (it.abilities && it.abilities->elementType != COMBAT_NONE && it.abilities->elementDamage != 0) {
					s << " fizyczne + " << it.abilities->elementDamage << ' ' << getCombatName(it.abilities->elementType);
				}
			}
			...
			if (it.abilities) {
				...
				if (it.abilities->stats[STAT_MAGICPOINTS]) {
					...
					s << "poziom magiczny " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
				}
				...
				if (show == 0) {
					...
					s << "full protection " << std::showpos << show << std::noshowpos << '%';
				}
				...
				if (it.abilities->speed) {
					...
					s << "predkosc chodzenia " << std::showpos << (it.abilities->speed >> 1) << std::noshowpos;
				}
			}
		...
		if (it.abilities) {
			...
			if (it.abilities->speed) {
				...
				s << "szybkosc " << std::showpos << (it.abilities->speed >> 1) << std::noshowpos;
			}
		}
		...
		if (!found) {
			if (it.isKey()) {
				s << " (Klucz:" << (item ? item->getActionId() : 0) << ')';
			} else if (it.isFluidContainer()) {
				if (subType > 0) {
					const std::string& itemName = items[subType].name;
					s << " napelnione " << (!itemName.empty() ? itemName : "czyms nieznanym");
				} else {
					s << ". Jest puste";
				}
			} else if (it.isSplash()) {
				...
				if (lookDistance <= 4) {
					if (item) {
						...
						if (!text->empty()) {
							...
							s << "Nic nie jest na tym napisane";
						}
					} else {
						s << "Nic nie jest na tym napisane";
					}
				} else {
					s << "Jestes zbyt daleko, aby to przeczytac";
				}
			} else if (it.levelDoor != 0 && item) {
				uint16_t actionId = item->getActionId();
				if (actionId >= it.levelDoor) {
					s << " dla poziomu " << (actionId - it.levelDoor);
				}
			}
		}
	}
	...
	if (it.showCharges) {
		s << " ktore ma " << subType << (subType != 1 ? " ladunkow" : " ladunek");
	}
	...
	if (it.showDuration) {
		if (item && item->hasAttribute(ITEM_ATTRIBUTE_DURATION)) {
			...
			if (duration >= 86400) {
				...
				s << days << (days != 1 ? " dni" : "dzien");

				if (hours > 0) {
					s << " i " << hours << (hours != 1 ? " godzin" : "godzine");
				}
			} else if (duration >= 3600) {
				uint16_t hours = duration / 3600;
				uint16_t minutes = (duration % 3600) / 60;
				s << hours << (hours != 1 ? " godzin" : "godzine");

				if (minutes > 0) {
					s << " i " << minutes <<  (minutes != 1 ? " minut" : "minute");
				}
			} else if (duration >= 60) {
				uint16_t minutes = duration / 60;
				s << minutes << (minutes != 1 ? " minut" : " minute");
				uint16_t seconds = duration % 60;

				if (seconds > 0) {
					s << " i " << seconds << (seconds != 1 ? " sekund" : " sekunde");
				}
			} else {
				s << duration << (duration != 1 ? " sekund" : " sekunde");
			}
		} else {
			s << " ktore sa nowka sztuka, niesmigana.";
		}
	}
	...
	if (it.wieldInfo != 0) {
		s << std::endl << "To moze byc prawidlowo noszone tylko przez ";
		...
		if (it.wieldInfo & WIELDINFO_LEVEL) {
			s << " z poziomem " << it.minReqLevel << " lub wyzszym";
		}
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			s << " poziomem magicznym " << it.minReqMagicLevel << " lub wyzszym";
		}
